...
function CatCard({ c, i }: { c: (typeof cats)[number]; i: number }) {
  ...
  return (
    ...
    >
      <div
        style={{
          background: "#fff", padding: 14, border: "1px solid rgba(28,24,20,0.15)",
      ...
      >
        <div
          style={{
            width: "100%", height: 220, marginBottom: 14,
            background: "repeating-linear-gradient(135deg, rgba(0,0,0,0.04) 0 6px, rgba(0,0,0,0.02) 6px 12px)",
            display: "flex", alignItems: "center", justifyContent: "center",
            color: "rgba(0,0,0,0.35)", fontFamily: "var(--mono)", fontSize: 10, letterSpacing: "0.06em",
          }}
        >
        ...
        <h3 className="m-0 mb-1.5" style={{ fontFamily: "var(--serif-display)", fontWeight: 500, fontSize: 24, lineHeight: 1.1 }}>
          {c.name}
        </h3>
        <p className="m-0 text-sm leading-snug" style={{ color: "rgba(28,24,20,0.7)" }}>{c.blurb}</p>
      </div>
    </Link>
  );
}
...
export function Categories() {
  return (
    <section id="visit" style={{ background: "var(--paper)" }}>
      <div className="section-inner" style={{ paddingTop: 100, paddingBottom: 100 }}>
        <div className="flex justify-between items-end mb-14">
          <h2
            className="m-0"
            style={{ fontFamily: "var(--serif-display)", fontWeight: 400, fontSize: 72, lineHeight: 0.95, letterSpacing: -1.6 }}
          >
            Five ways<br />into town.
          </h2>
