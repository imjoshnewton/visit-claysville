...
export function MapBlock({ pins = [], legend = "" }: { pins?: Pin[]; legend?: string }) {
  return (
    <section style={{ padding: "0 80px 100px", background: "var(--paper)" }}>
      <div className="grid items-stretch" style={{ gridTemplateColumns: "1fr 320px", gap: 32 }}>
        <div
          className="relative overflow-hidden"
          style={{ background: "#e8dfca", height: 440, border: "2px solid var(--ink)" }}
        >
          <svg
            viewBox="0 0 800 440"
          ...
          <div
            ...
            style={{ bottom: 14, right: 18, color: "var(--ink)", opacity: 0.65 }}
          >
            Main Street · National Pike
          </div>
        </div>
        <div style={{ background: "var(--ink)", color: "var(--paper)", padding: 28 }}>
          ...
          <h3
            className="m-0 mb-4"
            style={{ fontFamily: "var(--serif-display)", fontSize: 28, fontWeight: 500, lineHeight: 1.15 }}
          >
            {legend || "All within a half-mile of Main Street."}
          </h3>
          <div className="flex flex-col gap-4">
            {[
              ["From Pittsburgh", "45 min on I-70 W"],
              ["From Wheeling", "20 min on US-40 E"],
              ["From Washington", "15 min on US-40 W"],
              ["Free parking", "Town lot · Wheeling Pike"],
            ].map(([k, v]) => (
              <div
                key={k}
                className="flex justify-between pb-3"
                style={{ borderBottom: "1px solid rgba(243,235,220,0.2)" }}
              >
